...
$(document).ready(function(){
	$(window).scroll(function(){
		if($('.slider').offset().top < $('.navbar').offset().top) $('.navbar').css("background-color", "#000a38");
		if($('.slider').offset().top > $('.navbar').offset().top) $('.navbar').css("background-color", "transparent");
	})
});
...
$(document).ready(function () {
	...
	//arrow 클릭 시 slider wrapper 위치 변경
		if ($(this).hasClass("next")) {
			arrow_clickCount < sCount - 1 ? arrow_clickCount++ : (arrow_clickCount = 0);
		} else if ($(this).hasClass("prev")) {
			arrow_clickCount > 0 ? arrow_clickCount-- : (arrow_clickCount = sCount - 1);
		}
		TweenMax.to(sWrapper, 0.4, { x: "-" + $('.slide').innerWidth() * arrow_clickCount });
...
function nextInfo(e_title, e_txt, e_img, a_title, a_txt, a_img, cnt) {
	...
	if(img_clickCount >= cnt) img_clickCount = 0;
	else img_clickCount++;


	title.text(a_title[img_clickCount]);
	text.html(a_txt[img_clickCount]);
	image.attr("src", a_img[img_clickCount]);
}
...
//화면 크기 변경 시 slider wrapper 위치 조정
let delay = 1000;
let timer = null;
